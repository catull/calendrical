...
var calConstants, calendar, exp;

calConstants = require ('./calendrical.calendar.constants');
calendar = require ('./calendrical.calendar.conversions');
...
exp = (function () {
  ...
  function ephemerisCorrection (tee) {
    ...
    year = calendar.jdToGregorianYear (Math.floor (tee + calConstants.gregorian.EPOCH));

    if (year >= 1988 && year <= 2019) {
      return (year - 1933) / 86400;
    }

    centuries = (calendar.gregorianToJd (year, calConstants.JULY, 1) -
            calendar.gregorianToJd (1900, calConstants.JANUARY, 1)) / 36525;
    ...
    result = 0.5 + (calendar.gregorianToJd (year, calConstants.JANUARY, 1) -
               calendar.gregorianToJd (1810, calConstants.JANUARY, 1)
           );
  ...
  function julianCenturies (tee) {
    return (universalToDynamical (tee) - calConstants.J2000) / 36525;
  }
  ...
  function estimatePriorSolarLongitude (lambda, tee) {
    var rate = calConstants.MEAN_TROPICAL_YEAR / 360,
  ...
  function nthNewMoon (n0) {
    ...
    k0 = n0 - 24724;
    c0 = k0 / 1236.85;
    approx = calConstants.J2000 +
              poly (c0, [ 5.09766, calConstants.MEAN_SYNODIC_MONTH * 1236.85, 0.0001437, -0.000000150, 0.00000000073 ]);
  ...
  function lunarPhase (tee) {
    ...
    t0   = nthNewMoon (0);
    n0   = Math.round ((tee - t0) / calConstants.MEAN_SYNODIC_MONTH);
    phi2 = 360 * mod ((tee - nthNewMoon (n0)) /
              calConstants.MEAN_SYNODIC_MONTH, 1);
  ...
  function newMoonBefore (tee) {
    var t0 = nthNewMoon (0),
        phi = lunarPhase (tee),
        n0  = Math.round ((tee - t0) / calConstants.MEAN_SYNODIC_MONTH - phi / 360);
